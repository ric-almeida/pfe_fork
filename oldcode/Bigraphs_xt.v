...
Definition Finite {N I O} (p : N+I -> N+O) := forall n, Acc (fun n n' => p (inl n) = inl n') n.
...
Theorem finite_acyclic : forall {N I O : Type} (pp : N+I -> N+O),
  Finite pp -> forall n, ~closure pp (inl n) (inl n).
Proof.
unfold Finite.
intros N I O pp H n.
induction (H n).
...
Theorem finite_sequence : forall {P Q I M O : Type} (pp : P+I -> P+M) (pq : Q+M -> Q+O),
  Finite pp -> Finite pq -> Finite (sequence pp pq).
Proof.
unfold Finite.
intros.
destruct n as [p | q].
