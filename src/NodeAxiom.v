...
Definition renaming_tp {s r} 
  ...
  {Ha: ~In a N} {Ha': ~In a' N'}
  (b:bigraph s N r N') : 
    bigraph s {| ndlist := a::N; nd := NoDup_cons a Ha (nd N) |} r {| ndlist := a'::N'; nd := NoDup_cons a' Ha' (nd N') |}. 
  Proof.
...
Definition renaming (N N': NoDupList) {H: length N = length N'}: bigraph 0 N 0 N'.
  ...
  inversion H.
  apply IHN in H1.
  eset (almost := bigraph_tensor_product (dis_i:=_) (dis_o:=_) (elementary_renaming a a') H1).
  Abort.
...
Definition casinductif (n n':Name) {s i r o} (b: bigraph s i r o) 
  {Hn: ~In n i}
  {Hn': ~In n' o} : bigraph s (OneelNDL n ∪ i) r (OneelNDL n' ∪ o) := 
  bigraph_tensor_product 
